...
function agregarPresupuesto() {
  ...
  presupuestosArray.push(presupTemp);
  presupJson = JSON.stringify(presupuestosArray);
  localStorage.setItem('base', presupJson);
  arregloProductos = [];
  total = 0;
  let tabla = "<thead><tr><th>Repuesto</th><th>Precio</th></tr></thead><tbody>";
  document.getElementById("repuestos").innerHTML = tabla;
...
function hacerTablaPresupuestos(presArray) {
  cantidad = presupuestosArray.length;
  let tabla2 = "<thead><tr><th>#</th><th>Cliente</th><th>Total</th></tr></thead><tbody>";
  for (let i = 0; i < cantidad; i++) {
    tabla2 += `<tr><td>${presArray[i].numero}</td><td>${presArray[i].cliente}</td><td>${presArray[i].total}</td></tr>`;
  }

  document.getElementById("presupuestos").innerHTML = tabla2;
}
